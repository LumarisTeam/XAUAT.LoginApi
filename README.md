...
| 10 | `/login/{u}/{p}` 失败**仍返回 200** | 这不是差异，是**刻意保留**的 Flask 历史怪癖：老客户端可能按"200 + success 字段"判断，改成 401 会让它们把失败当成功 |
...
Flask 侧对 CAS 登录页选择器、`studentExamInfoVms` 正则、AES 加密**零测试覆盖、零真实样本**。
本仓库的解析器是按 Flask 代码逆推写的，虽然逻辑逐行对齐，但**尚未用真实响应验证过**。
...
脚本会复刻 Flask 的请求序列抓一遍上游，把响应原文（脱敏后）落到
`XAUAT.LoginApi.Tests/TestFixtures/`，并输出一份 `capture-report.txt` 记录每步的
状态码 / Content-Type / 字符集 / 重定向链——这几项直接决定编码处理与换票逻辑是否正确。
**切换生产之前请务必先跑一次。**
